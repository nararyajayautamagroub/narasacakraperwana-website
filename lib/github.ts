...
export async function getOwnedRepositories(includePrivate=false):Promise<GitHubRepo[]>{
  ...
  let repos:any[]=token
    ? (await githubFetch<any[]>(`/user/repos?affiliation=owner&per_page=100&sort=updated`)||[])
    : (await githubFetch<any[]>(`/users/${encodeURIComponent(owner)}/repos?per_page=100&sort=updated`)||[]);
  repos=repos.filter(r=>includePrivate||!r.private);
  const enriched=await Promise.all(repos.map(async r=>{
    const [commits,runs]=await Promise.all([
      githubFetch<any[]>(`/repos/${r.full_name}/commits?sha=${encodeURIComponent(r.default_branch)}&per_page=1`),
      githubFetch<any>(`/repos/${r.full_name}/actions/runs?per_page=1`)
    ]);
    const c=commits?.[0];
    const w=runs?.workflow_runs?.[0];
    return {
      id:r.id,name:r.name,full_name:r.full_name,html_url:r.html_url,private:!!r.private,archived:!!r.archived,fork:!!r.fork,
      default_branch:r.default_branch,description:r.description||null,language:r.language||null,stargazers_count:r.stargazers_count||0,
      forks_count:r.forks_count||0,open_issues_count:r.open_issues_count||0,updated_at:r.updated_at,pushed_at:r.pushed_at||null,size:r.size||0,
      last_commit:c?{sha:c.sha,message:String(c.commit?.message||"").split("\n")[0],author:c.commit?.author?.name||null,date:c.commit?.author?.date||c.commit?.committer?.date||""}:null,
      latest_workflow:w?{status:w.status,conclusion:w.conclusion||null,name:w.name||null,html_url:w.html_url||null,updated_at:w.updated_at}:null
